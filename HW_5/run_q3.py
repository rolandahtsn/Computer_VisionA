...
h_ = forward(test_x,saved_params,'layer1',activation=sigmoid)
probs = forward(h_,saved_params,'output',activation=softmax)
for i in range(0,test_y.shape[0]):
    y = np.argmax(test_y[i])
    pr = np.argmax(probs[i])
    confusion_matrix[y,pr]+=1
